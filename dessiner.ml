...
let dessiner_image_sur_image img1 img2 x y =
  let (largeur,hauteur) = dimensions img1 in
  let fond = Graphics.dump_image (img2 x y largeur hauteur) in
  let melanger_tordu (c1,o) c2 = melanger c1 c2 o in
  Graphics.draw_image (Graphics.make_image (matrixmap2 melanger_tordu img1 fond)) x y;;
